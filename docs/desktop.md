...
## Communication
- Renderer calls `window.patcherDesktop.invoke(method, payload)`.
- Electron main handles `patcher:invoke` via `ipcMain.handle`.
- No localhost port, no Express process, no HTTP polling mismatch from process split.

## Task Flow
1. UI sends `startTask` through IPC.
2. Desktop service spawns `node cli/main.js ...` child process.
...
- 啟動任務時，Desktop 會以環境變數 `PATCH_KEYSTORE_PATH` 傳入目前選擇值，確保本次任務與 UI 一致。
